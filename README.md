...
pi 的输入框本来就支持用 ↑↓ 翻看本次会话敲过的内容（上限 100 条），但它**只活在内存里**：退出 pi、`/reload`、`/new`、`/resume` 之后就没了，也不区分工作目录。本扩展只补一件事——把它**存到磁盘**，下次启动时重新喂回 pi 的历史列表。↑↓ 的触发规则、草稿保护、去重与 100 条上限全部沿用 pi 自己的实现。
...
- **不记录**：斜杠命令（`/model`、`/endpoint`、`/input-history` …）、`!bash` / `!!bash`、rpc 客户端与扩展注入的消息，以及 pi 打开旧会话时的历史重放。
...
- 斜杠命令与 `!bash` **结构性地不进**输入历史文件（见 [输入历史](#输入历史)）。
