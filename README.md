...
- 子代理的**费用**只在上游报了金额时才有：事件或 `_meta.json` 里的 `cost` / `modelAttempts[].usage.cost` 会原样采用；只报到 token（`totalTokens` / `tokens`）、或只能靠扫子会话 `session.jsonl` 兜底时，**token 照记、费用记 0**，不按父模型的费率倒推。所以 `/calls` 里子代理行的费用偏低是预期行为，不代表 token 漏算。
