...
| `/calls` | 查看本轮或本会话的 per-model 用量与费用明细（TUI 为交互菜单；rpc 等非 TUI 会话直接回一段文本摘要） |
| `/reload` | 安装或更新插件后重载扩展 |
...
- 远程网关须使用 **HTTPS**；HTTP 仅允许 loopback（`localhost`、`127.0.0.0/8`、`::1`、IPv4-mapped loopback）。无 insecure 覆盖开关。被拒绝的 baseUrl 会打印一条 warning 说明原因（URL 中的凭证会被脱敏，同一原因只报一次），避免只看到「LLMGates is not configured」而无从排查。
