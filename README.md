...
pi install npm:@llmgates_api/pi-llmgates-provider@0.4.0    # 指定版本
...
- 记的是**显式切换**：pi 恢复会话自己的模型时不发 `model_select`，所以 `pi -c` 打开的老会话用的那个模型**不会**被记下来。因此从老会话直接 `/new`，回到的是上一次显式切过的模型，而不是刚才那个。
