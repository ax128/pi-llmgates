...
- **修复默认用量账本的质量、归属与刷新开销。** 缺失指标参与汇总降级；状态行、`/calls` 标题及明细统一保留 `≥N`、`?`、`~` 与已知小计 `+ ?`。工具/压缩的费用来源不再被 legacy 归一化抹掉。恢复多模型 `modelAttempts` 分区，保留并行 child 的进度身份；meta/tool 按各自 revision 去旧，再按接收顺序替换。Coverage 改用每 producer 序号，内存容量缺口与磁盘耗尽分开；未变化的账本投影缓存复用。持久化仍默认关，目录遍历、保留期清理与 checkpoint 容量问题保留为后续专项。
