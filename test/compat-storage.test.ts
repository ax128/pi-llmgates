...
describe("compat registry storage", () => {
	...
	it.each([
		...
		[
			"malformed entry",
			JSON.stringify({ instances: [{ ...first, scheme: "unknown" }] }),
		],
	...
	it("conditionally replaces only an unchanged registry entry", async () => {
		...
		try {
			...
			await expect(
				replaceInstanceIfEqual(agentDir, first, renamed),
			).resolves.toEqual(renamed);
			await expect(
				replaceInstanceIfEqual(agentDir, first, {
					...first,
	...
	it("rejects duplicate IDs case-insensitively", async () => {
		...
		try {
			...
			await expect(
				addInstance(agentDir, { ...second, id: "WORK-NEWAPI" }),
	...
	it("preserves both concurrent adds", async () => {
		...
		try {
			await Promise.all([
				addInstance(agentDir, first),
				addInstance(agentDir, second),
			]);
			expect(listInstances(agentDir)).toEqual(
...
describe("compat refresh metadata", () => {
	it("round-trips versioned metadata and rejects malformed metadata", () => {
		const encoded = encodeCompatRefreshMeta({
		...
		expect(decodeCompatRefreshMeta(encoded)).toEqual({
			baseUrl: first.baseUrl,
			scheme: first.scheme,
		});
		for (const raw of [
...
describe("provider auth storage", () => {
	it("treats missing auth as empty, preserves siblings, and writes mode 0600", async () => {
		...
		try {
			...
			await writeProviderOAuthCredential(
				agentDir,
				"sibling",
				credential("sibling-key", { ...first, id: "sibling" }),
			);
			await writeProviderOAuthCredential(
				agentDir,
				first.id,
				credential("literal !$HOME ${HOME}"),
			);
			...
			await expect(
				assertAuthEntryAbsent(agentDir, "WORK-NEWAPI"),
	...
	it("conditionally deletes a case-insensitive auth key only for an exactly equal credential", async () => {
		...
		try {
			...
			expect(
				await deleteProviderAuthEntryIfEqual(agentDir, "WORK-NEWAPI", original),
			).toBe(true);
			expect(readJson(path)).toEqual({});
		} finally {
			cleanup();
	...
	it.each([
		...
		"fails closed on auth with %s, reports a fixed safe error, and does not overwrite it",
		async (_label, raw) => {
			const { agentDir, cleanup } = withTempAgentDir();
			const path = join(agentDir, "auth.json");
			...
			try {
				...
				for (const operation of operations) {
					...
					await expect(operation()).rejects.not.toThrow(
						/storage-sentinel-secret|"access"|broken/i,
					);
				}
				expect(readFileSync(path, "utf8")).toBe(raw);
			} finally {
	...
	it.each([
		[
			...
			/access|blank|empty/i,
		],
		[
			"invalid refresh",
			{ ...credential("secret"), refresh: "{}" },
			/refresh|metadata/i,
		],
	...
	it("forces the stored credential type to oauth", async () => {
		const { agentDir, cleanup } = withTempAgentDir();
		try {
			await writeProviderOAuthCredential(agentDir, first.id, {
				...credential("secret"),
				type: "api_key",
			} as unknown as OAuthCredential);
			expect(
				(readJson(join(agentDir, "auth.json"))[first.id] as { type: string })
					.type,
			).toBe("oauth");
		} finally {
			cleanup();
	...
	it("rejects invalid instance provider IDs", async () => {
		...
		try {
			await expect(
				writeProviderOAuthCredential(agentDir, "bad/id", credential("secret")),
			).rejects.toThrow(/instance id/i);
		} finally {
			cleanup();
		}
	});
	...
	it("conditional compensation does not delete a credential changed by another writer", async () => {
		...
		const replacement = {
			...credential("replacement"),
			validationNonce: "new-writer",
		};
		try {
			await writeProviderOAuthCredential(agentDir, first.id, original);
			writeJson(join(agentDir, "auth.json"), { [first.id]: replacement });
			expect(
				await deleteProviderAuthEntryIfEqual(agentDir, first.id, original),
			).toBe(false);
			expect(readJson(join(agentDir, "auth.json"))[first.id]).toEqual(
				replacement,
			);
			expect(
				await deleteProviderAuthEntryIfEqual(agentDir, first.id, replacement),
			).toBe(true);
		} finally {
			cleanup();
		}
