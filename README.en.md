...
pi install npm:@llmgates_api/pi-llmgates-provider@0.7.0    # pinned version
...
Meta and tool revisions are checked within their own inlet, then accepted snapshots replace the shared ledger contribution in session arrival order. Millisecond mtimes are never compared with tool counters. Completion events without revisions retain the existing deduplication boundary. Parallel tool progress retains each child's identity; each usage-bearing progress snapshot replaces that tool's previous progress, and the final result replaces progress again. Coverage sequences advance independently per producer. At the 10,000-observation memory limit, if no provisional record can be evicted, Coverage shows `partial · memory-exhausted` separately from disk `storage-exhausted`, and known totals become lower bounds. Unchanged ledger projections are cached for the 1s footer refresh.
...
- async / background subagents are collected through the `subagent:async-complete` / `subagent:foreground-complete` event bypass: the numbers come from the `usage` / `modelAttempts` / `totalCost` / `tokens` the event carries, and from the `_meta.json` files in the directories above when it carries none. pi-subagents 0.69 `bg_wait` is a management projection: only completion children whose run IDs were already observed through the current session's trusted path are accepted; pooled top-level `usage` and `details.results` are ignored, and a wait result cannot bind a new run. Parent-run plus flat child index forms the normal identity; an indexless meta file maps to index 0 only when that parent/agent has no indexed sibling, otherwise it is fail-closed. In-flight `tool_execution_update` payloads that already carry usage are ingested too; `tool_execution_end` for the same `toolCallId` replaces that in-flight progress even when the two payloads mint different sourceKeys. A newer `_meta.json` (mtime advanced) replaces **that file's** previous snapshot — a sibling dropped for cross-granularity reasons is not brought back because the file grew. **A completion event that already occupies the same `sourceKey` is not replaced by a later meta file** (first-wins, to avoid double counting); meta is waited on only when the event carried no numbers. **The `status.json` under pi-subagents' temp root and the child `session.jsonl` are not read** — in the default layout both sit outside the workspace (asyncDir under `os.tmpdir()`, child sessions under `~/.pi/`), and those two fallbacks were only ever allowed to read paths inside the workspace, so they never fired; they and that guard have been removed.
...
- A subagent row carries a **cost** only when the upstream reported money: the first of `usage.cost`, the sum of `modelAttempts[].usage.cost`, and `totalCost.costUsd` that has a value is taken as-is. When only tokens came through (`tokens` / `totalTokens`), **the tokens are counted and cost is shown as unknown (`?`)** rather than estimated at the parent model's rate. A subagent row with a low cost in `/calls` is therefore expected, not a lost token count.
- Any tool that follows pi's convention of hanging a top-level `usage` on its result is counted — this is not tied to any one extension. A result that names its own model gets a `<provider>/<model>` row, merging with the parent model's row when the name matches; one that names no model lands under `tool/<toolName>`, with `?` for cost unless it reports a verifiable amount. Pi-compatible numeric `usage.cost` and a complete `{input, output, cacheRead, cacheWrite, total}` object (including an explicit zero total) are reported as producer-supplied cost; a missing cost is estimated only when a known pricing rule/registry entry matches. Unknown model/provider IDs never receive a default rate, and malformed or partial cost objects remain unknown. Tool names already claimed by the subagent path, or whose usage would double-count, are excluded: `subagent`, `task`, `bg_wait`, `subagent_wait`, `subagent_supervisor`, `intercom`, plus `@tintinweb/pi-subagents`' `Agent` / `get_subagent_result` / `steer_subagent`.
- Two deliberate under-counts come with that. The three `@tintinweb/pi-subagents` names stay excluded; `subagents:completed` / `subagents:failed` are fail-closed probes that mark Coverage `unavailable` and **do not enter All**. If you have manually enabled that package's `reportUsage` (off by default), its usage is not counted in the totals. And a tool result that pools several LLM calls without reporting a count keeps **calls unknown** (the parser's defaulted `1` is not treated as an exact count); its tokens and cost are unaffected. Under-counting is the safe direction; double counting is not.
...
- Aggregation is ordered through an asynchronous task chain and makes no additional model calls; counting happens only in an interactive parent session (TUI). Without persistence, reload/restart does not restore usage. Third-party runners and external CLIs are **not a support list**: Coverage marks them `unavailable` / unverified (EventBus probes are fail-closed and usage-shaped payloads do not enter All). **An empty session pre-lists three S4 placeholder rows for CLI/job/runs** (not installed runners); npm probes such as tintinweb appear only after their events arrive. Directory hits in the research inventory are not compatibility certifications.

Persistence remains opt-in: each append synchronously checks the entire usage directory, and closed roots are not automatically pruned by retention age. When the complete checkpoint exceeds 256KiB, writing is skipped while the old checkpoint/journal is retained; long sessions can eventually reach the 8MiB per-root or 64MiB global limit. Write retries, the orphan queue, and per-tick read budgets are not implemented. These limitations require a separate persistence change before enabling persistence by default; automatic rotation and write recovery are not promised.
...
| File | Contents |
| --- | --- |
| `config.json` | Extension-level switches: `pricingAutoUpdate`, `inputHistory`, `inputHistoryScope`, `restoreLastModel`, `tps`, `tpsPersist`, `tpsExt` |
...
- `tps` / `tpsPersist` / `tpsExt` are described under [Status line and `/calls`](#status-line-and-calls). Environment variables override the file; when the master switch is off, `tpsPersist` does not write. A hand edit needs a `/reload` to take effect.
...
| `LLMGATES_TPS` | Master usage-collection switch (enabled by default; `0` / `false` / `no` stops every inlet, including persist writes) |
